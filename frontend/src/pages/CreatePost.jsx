...
const CreatePost = () => {
    ...
    const [descriptionFilled, setDescriptionFilled] = useState(true); // Add state for description field
    ...
    // Update description field filled state
                        ...
                        <div className='flex px-4 mt-3'>
                            {cats?.map((c, i) => (
                                <div key={i} className='flex justify-center items-center space-x-2 mr-4 bg-gray-200 px-2 py-1 rounded-md'>
                                    <p>{c}</p>
                                    <p className='bg-black text-white rounded-full cursor-pointer p-1 text-sm' onClick={() => deleteCategory(i)}><ImCross /></p>
                                </div>
                            ))}
                        </div>
                    </div>
                    <textarea onChange={handleDescriptionChange} rows={15} cols={30} className={`px-4 py-2 outline-none ${!descriptionFilled && 'border-red-500'}`} placeholder='Enter post description' />
